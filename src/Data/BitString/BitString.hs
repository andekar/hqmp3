...
-- This library provides a pure interface to bits, similar to the monadic BitGet
-- library. This version uses lazy bytestrings.
...
splitAt i bs@(Chunk lb j rest)
    ...
    | atLeast bs (i - (8-j)) = let (f, s) = splitAt (i - (L.length lb - j)) rest
                               in  (Chunk lb j f, s)
    | otherwise = (bs, Empty)
  where
    fstLen          = (i + j + 7) `div` 8
